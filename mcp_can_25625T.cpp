...
INT8U MCP_CAN::mcp2515_readRegister(const INT8U address)                                                                     
{
    ...
    // #if DEBUG_MODE  then print out all values and labels for debugging
    #if DEBUG_MODE
...
void MCP_CAN::mcp2515_modifyRegister(const INT8U address, const INT8U mask, const INT8U data)
{
...
#if DEBUG_MODE
    Serial.print(F("Modified Register Address: "));
    Serial.print(address, HEX);
    Serial.print(F(", Mask: "));
    ...
    Serial.println(mcp2515_readRegister(address), HEX);
#endif
}
...
INT8U MCP_CAN::mcp2515_setCANCTRL_Mode(const INT8U newmode)
{
    ...
    // Request the new mode and check for success
    INT8U res = mcp2515_requestNewMode(newmode);
    if (res != MCP2515_OK)
    {
#if DEBUG_MODE
        Serial.print(F("Failed to set CANCTRL Mode to: "));
        Serial.println(newmode, HEX);
        Serial.print(F("Current Mode: "));
        Serial.println(mcp2515_readRegister(MCP_CANSTAT) & MODE_MASK, HEX);
#endif
    }
    else
    {
#if DEBUG_MODE
        Serial.print(F("Successfully set CANCTRL Mode to: "));
        Serial.println(newmode, HEX);
#endif
    }
    return res;
...
** Descriptions:            Request new mode
*********************************************************************************************************/
INT8U MCP_CAN::mcp2515_requestNewMode(const INT8U newmode)
{
    mcp2515_modifyRegister(MCP_CANCTRL, MODE_MASK, newmode);
    INT8U mode = mcp2515_readRegister(MCP_CANCTRL) & MODE_MASK;

    // Wait for mode change to complete
    unsigned long startTime = millis();
    while ((mode != newmode) && (millis() - startTime < 200)) // Timeout after 200 ms
    {
        mode = mcp2515_readRegister(MCP_CANSTAT) & MODE_MASK;
    }
    ...
    {
#if DEBUG_MODE
        Serial.print(F("Mode change failed. Current mode: "));
...
INT8U MCP_CAN::mcp2515_init(const INT8U canIDMode, const INT8U canSpeed, const INT8U canClock)
{
  ...
  INT8U res;
    ...
    {
#if DEBUG_MODE
      Serial.println(F("Entering Configuration Mode Failure  mcp2515_init...")); 
#endif
      return res;
    }
#if DEBUG_MODE
...
INT8U MCP_CAN::init_Mask(INT8U num, INT8U ext, INT32U ulData)
{
    ...
    if(res > 0){
#if DEBUG_MODE
    Serial.println(F("Entering Configuration Mode Failure init_Mask...")); 
#endif
	return res;
     }
